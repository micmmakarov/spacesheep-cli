...
async function child(jobFile) {
  ...
  if (job.event !== "tool") {
    ...
    if (job.source === "antigravity") { if (job.transcript) { const { started_at, last_at, ...f } = antigravityFacts(job.transcript); Object.assign(body, f); } if (job.model) body.model = job.model; }
    else if (job.transcript) { const { started_at, last_at, ...f } = claudeFacts(job.transcript, job.session_id); Object.assign(body, f); }
...
function collectBackfill(dirs, which = { claude: true, codex: true, antigravity: true }) {
  ...
  if (which.antigravity) for (const data of AG_DATA) {
    ...
    for (const id of ids) {
      ...
      if (!facts.started_at) continue;
      const row = { source: "antigravity", session_id: id, machine, ...facts };
      if (ssh) row.ssh = ssh;
      rows.push(row);
    }
  }
...
async function install(opts, log, argv) {
  ...
  if (both || opts.claude || opts.codex) log(opts.noMemory
    ? `  --no-memory: only session state and metadata are sent, never what was said.`
    : `  Turn sync is on too: each finished Claude Code / Codex turn's text goes to your spacesheep memory. --no-memory turns it off.`);
...
function status(opts, out) {
  ...
  const report = { machine: machineName(), ssh: cfg.readConfig().ssh || null, accounts, antigravity, running_sessions: running, last_ping: last, board: appOrigin() + "/sessions" };
...
module.exports = { ping, install, uninstall, status, backfill, forget, collectBackfill, claudeFacts, codexFacts, antigravityFacts, antigravityEntry, agWorkspace, firstAsk };
