...
function collectBackfill() {
  ...
  for (const f of recentFiles(CLAUDE_PROJECTS, 1, since, (n) => /^[0-9a-f-]{36}\.jsonl$/.test(n))) {
...
function readSettings() {
  try { return JSON.parse(fs.readFileSync(CLAUDE_SETTINGS, "utf8")); } catch (_) { return {}; }
}

function installClaude(bin, log) {
  ...
  log(`  ${added.length ? "✓" : "="} Claude Code: session hooks ${added.length ? `added (${added.join(", ")})` : "already there"} in ${CLAUDE_SETTINGS}`);
}

function uninstallClaude(log) {
  const settings = readSettings();
  if (!settings.hooks) return log("  = Claude Code: session hooks were not installed");
  let changed = false;
  ...
  log(`  ${changed ? "✓" : "="} Claude Code: session hooks ${changed ? "removed" : "were not installed"}`);
}
...
async function install(opts, log, argv) {
  ...
  log(`  Machine: ${machineName()}${c.ssh ? ` (ssh ${c.ssh})` : ""}`);
  if (both || opts.claude) installClaude(bin, log);
  // Turns and Codex's single notify are the memory hook's; it forwards Codex turns here.
  mem.install({ claude: opts.claude, codex: opts.codex, quiet: true }, log);
  await backfill(log);
  log(`\n  Open https://spacesheep.dev/sessions. Sessions you start from now on show up live; sessions already running show up once they restart.`);
}
...
function status(opts, out) {
  ...
  const wired = Object.keys(HOOKS).filter((ev) => (s.hooks && s.hooks[ev] || []).some(isOurs));
  let last = null;
  try { last = JSON.parse(fs.readFileSync(path.join(STATE, "last.json"), "utf8")); } catch (_) {}
  const report = { machine: machineName(), ssh: cfg.readConfig().ssh || null, claude_hooks: wired, last_ping: last, board: appOrigin() + "/sessions" };
  if (opts.json) return out(report);
  out(`  machine:      ${report.machine}${report.ssh ? ` (ssh ${report.ssh})` : ""}`);
  out(`  Claude Code:  ${wired.length === Object.keys(HOOKS).length ? "session hooks installed" : wired.length ? `partly installed (${wired.join(", ")})` : "not installed"}`);
