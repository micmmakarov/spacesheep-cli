...
async function child(jobFile) {
  ...
  try {
    ...
    for (let i = 0; i < turns.length; i += BATCH) {
      ...
      const ok = await post(k.key, { source: job.source, session_id: job.sessionId, cwd: job.cwd, turns: batch.map(({ endLine, ...t }) => t) });
...
function hookEntry(bin) {
  return { hooks: [{ type: "command", command: `${bin} memory sync`, timeout: 10 }] };
}
...
function installCodex(bin, log) {
  const line = `notify = [${JSON.stringify(bin)}, "memory", "sync", "--source", "codex"]`;
  let text = "";
  try { text = fs.readFileSync(CODEX_CONFIG, "utf8"); } catch (_) {}
  const m = text.match(/^\s*notify\s*=\s*(.+)$/m);
  if (m && /spacesheep(\.js)?", *"memory", *"sync"/.test(m[1])) return log(`  = Codex: notify already set in ${CODEX_CONFIG}`);
  if (m) {
...
function uninstallCodex(log) {
  ...
  if (next !== text) { fs.writeFileSync(CODEX_CONFIG, next); log(`  ✓ Codex: notify removed`); }
  else log(`  = Codex: notify was not ours`);
}
...
function status(opts, out) {
  ...
  let codex = false;
  try { codex = /^\s*notify\s*=.*spacesheep(\.js)?", *"memory", *"sync"/m.test(fs.readFileSync(CODEX_CONFIG, "utf8")); } catch (_) {}
  const synced = {};
  ...
  const report = { signed_in: !!k, claude_code: claude, codex, synced, state_dir: STATE, app_origin: APP_ORIGIN };
